...
Page({
      ...
      success: function (res) {
        ...
        if (res.data.status === 0) {
          for (var i = 0; i < res.data.ds.length; i++) {
            that.setData({
              id: res.data.ds[i].id,
              uname: res.data.ds[i].userName,
              corporateName: res.data.ds[i].corporateName,
              positionName: res.data.ds[i].positionName,
              phoneNum: res.data.ds[i].phoneNum,
              address_txt: res.data.ds[i].address_txt,
              city: res.data.ds[i].city,
              province: res.data.ds[i].province,
              address_Details: res.data.ds[i].address_Details
            })
          }
  ...
  bindMultiPickerChange: function (e) {
    ...
    that.setData({
      ...
      city: that.data.multiArray[0][e.detail.value[0]],
      province: that.data.multiArray[1][e.detail.value[1]]
    })
  },
